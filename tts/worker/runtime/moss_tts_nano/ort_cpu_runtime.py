...
"""Adapted from the upstream OpenMOSS/MOSS-TTS-Nano ONNX CPU runtime."""
    ...
    def generate_audio_frames(
        ...
        for step_index in range(int(generation_defaults["max_new_frames"])):
            ...
            if "local_greedy_frame" in self.sessions and not bool(generation_defaults["do_sample"]):
                ...
                local_text_logits, _, local_past_by_name = self.run_local_cached_step(
                    global_hidden,
                    text_token_id=0,
                    audio_token_id=0,
                    channel_index=0,
                ...
                if next_text_token != int(self.manifest["tts_config"]["audio_assistant_slot_token_id"]):
                    break
                _, audio_logits, local_past_by_name = self.run_local_cached_step(
                    global_hidden,
