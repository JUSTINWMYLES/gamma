<script lang="ts">
  /**
   * InstructionSlideshow.svelte
   *
   * A themed slide-based instruction presentation that replaces the old
   * flat bullet-list instructions. Features:
   *
   * - Full-screen slides with unique gradients, colors, and fonts per game
   * - Auto-advance with configurable timing
   * - Smooth crossfade transitions between slides
   * - Progress dots at the bottom
   * - Two modes: "player" (shows Got it! button) and "viewer" (shows ready counter)
   * - Placeholder structure for future voice narration
   */
  ...
  let slideProgress = 0; // 0-1 progress through current slide
  ...
  function goToSlide(index: number) {
    ...
    // Brief delay for fade-out, then switch
    setTimeout(() => {
      currentIndex = index;
...
>
  ...
  <!-- Navigation arrows (subtle, for manual control) -->
  {#if slides.length > 1}
  ...
  <div class="absolute bottom-0 inset-x-0 z-20 flex flex-col items-center gap-4 pb-6 px-6">
    ...
    {#if slides.length > 1}
      ...
      <!-- Auto-advance progress bar (thin line under dots, only on non-last slides) -->
      {#if !isLastSlide}
