...
<div class="flex-1 flex flex-col items-center justify-center gap-6 p-6" data-testid="host-screen">
  ...
  <div class="w-full max-w-xs space-y-4">
    ...
    {#if localError}<p class="text-red-400 text-sm text-center">{localError}</p>{/if}
    <button class="w-full py-4 rounded-xl text-lg font-bold bg-indigo-600 hover:bg-indigo-500 active:scale-95 transition-all {submitting ? 'opacity-50 pointer-events-none' : ''}" on:click={handleHost} data-testid="host-btn">{submitting ? "Creating..." : "Create Room"}</button>
    <button class="w-full py-3 rounded-xl text-sm text-gray-500 hover:text-gray-300 transition-colors" on:click={() => dispatch("back")}>Back</button>
  </div>
</div>
