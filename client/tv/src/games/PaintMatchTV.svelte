<script lang="ts">
  /**
   * Paint Match — TV-only viewer component.
   *
   * Displays the target colour prominently, a countdown timer,
   * submission progress, and round results with rankings.
   */
...
<div class="flex-1 flex flex-col bg-gray-950" data-testid="paint-match-tv">
  {#if showResults && rankings.length > 0}
    <div class="flex-1 flex flex-col items-center justify-center gap-8 p-8">
      <div class="text-center">
        ...
        <div class="w-32 h-32 rounded-2xl border-4 border-amber-500 mx-auto shadow-2xl"
      ...
      <div class="flex flex-wrap justify-center gap-4 max-w-4xl">
        {#each rankings as result}
          <div class="flex flex-col items-center gap-2 p-4 rounded-xl bg-gray-900 border
            ...
            <div class="w-16 h-16 rounded-lg border-2 border-gray-600"
              style="background:rgb({result.mixRGB[0]},{result.mixRGB[1]},{result.mixRGB[2]})"></div>
  ...
  {:else if targetRGB}
    <div class="flex-1 flex flex-col items-center justify-center gap-6 p-8">
      ...
      <div class="w-80 h-80 rounded-3xl border-4 border-white/20 shadow-2xl transition-colors"
        style="background:rgb({targetRGB[0]},{targetRGB[1]},{targetRGB[2]})"></div>
      <p class="text-sm font-mono text-gray-500">RGB({targetRGB[0]}, {targetRGB[1]}, {targetRGB[2]})</p>
      <p class="text-6xl font-black font-mono {timeLeft < 10 ? 'text-red-400' : 'text-white'}">{Math.ceil(timeLeft)}</p>
      <div class="flex items-center gap-3">
        <div class="w-48 h-3 bg-gray-800 rounded-full overflow-hidden">
          <div class="h-full bg-green-500 rounded-full transition-all"
            style="width:{totalPlayers > 0 ? (submitCount / totalPlayers) * 100 : 0}%"></div>
        </div>
