<script lang="ts">
   ...
   * TV Lobby screen.
   *
   * Shows the 3-step setup flow (location → activity → display) before
   * the game picker. Once setup is complete, games are filtered by criteria
   * with unavailable games shown grayed out.
   */
  ...
  $: isHost = true; // TV is always host in this flow

  // ── Start setup on first render if not yet started ────────────────────────
...
<div class="flex-1 flex flex-col items-center justify-center gap-8 p-10">
    ...
    <div class="w-full max-w-3xl">
      ...
      {#if setupStep === 1}
        ...
        <div class="grid grid-cols-2 gap-6">
          <button
            class="flex flex-col items-center gap-4 p-8 rounded-2xl border-2 border-gray-700 bg-gray-800 hover:border-indigo-500 hover:bg-indigo-900 transition-colors"
  ...
  <!-- ── Main lobby (setup complete) ───────────────────────────────── -->
  {:else}
    <div class="w-full max-w-5xl grid grid-cols-2 gap-8">

      <!-- Left: player list + setup summary -->
      <div class="space-y-4">
        <!-- Setup summary badge -->
        <div class="flex gap-2 flex-wrap">
          ...
          <button
            class="px-3 py-1 bg-gray-700 rounded-full text-xs text-gray-400 hover:text-white transition-colors"
            on:click={resetSetup}
          >Change setup</button>
        </div>
              ...
              >
                <div class="flex items-start justify-between gap-2">
                  ...
                  {#if unavailableReason}
                    <span class="text-xs bg-gray-800 border border-gray-600 rounded px-2 py-0.5 flex-shrink-0 text-gray-500 mt-0.5">
                      {unavailableReason}
                    </span>
                  {/if}
                </div>
