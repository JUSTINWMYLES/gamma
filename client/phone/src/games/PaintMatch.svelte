<script lang="ts">
  /**
   * Paint Match — Phone-only player component.
   *
   * Identical in functionality to the app version. Shows five paint bucket
   * sliders, a live colour preview, and a submit button.
   */
...
<div class="flex-1 flex flex-col bg-gray-950 select-none" data-testid="paint-match-player">
  ...
  <div class="px-4 py-2 bg-gray-900 flex items-center justify-between">
    <span class="text-xs text-gray-400">Round {state.currentRound}/{state.gameConfig.roundCount}</span>
    <div class="flex items-center gap-3">
      <span class="text-xs text-gray-400">{submitCount}/{totalPlayers} submitted</span>
      <span class="text-xl font-mono font-black {timeLeft < 10 ? 'text-red-400' : 'text-white'}">{Math.ceil(timeLeft)}</span>
    </div>
  </div>
  ...
  {#if showResults}
    <div class="flex-1 flex flex-col items-center justify-center gap-4 p-4">
      <p class="text-lg font-bold text-white">
        {#if myRank === 1}You got the closest match!{:else if myRank <= 3}Nice try! #{myRank}{:else}You placed #{myRank}{/if}
      </p>
      <div class="flex gap-4 items-center">
        <div class="text-center">
          <p class="text-xs text-gray-400 mb-1">Target</p>
          <div class="w-16 h-16 rounded-lg border-2 border-gray-600"
            style="background:rgb({targetRGB?.[0] ?? 0},{targetRGB?.[1] ?? 0},{targetRGB?.[2] ?? 0})"></div>
        </div>
        <div class="text-2xl text-gray-500">vs</div>
        <div class="text-center">
          <p class="text-xs text-gray-400 mb-1">Your mix</p>
          <div class="w-16 h-16 rounded-lg border-2 border-gray-600"
            style="background:{submittedColor || previewColor}"></div>
        </div>
      </div>
    ...
    <div class="flex-1 flex flex-col p-4 gap-3 overflow-y-auto">
      <div class="flex gap-3 items-stretch">
        <div class="flex-1 flex flex-col items-center gap-1">
          ...
          <div class="w-full aspect-square rounded-xl border-2 border-gray-600 transition-colors"
            style="background:{previewColor}"></div>
        </div>
        <div class="flex flex-col items-center gap-1">
          <p class="text-xs text-gray-400">Target</p>
          <div class="w-20 aspect-square rounded-xl border-2 border-amber-500/50"
            style="background:rgb({targetRGB[0]},{targetRGB[1]},{targetRGB[2]})"></div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2">
        {#each buckets as bucket}
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full border-2 border-gray-600 flex-shrink-0"
              style="background:{bucket.color}"></div>
      ...
      {#if !submitted}
        <button class="mt-2 w-full py-3 rounded-xl font-bold text-lg
          bg-gradient-to-r from-pink-600 to-purple-600 text-white
          active:scale-95 transition-transform" on:click={handleSubmit}>
          Submit Mix
        </button>
      {:else}
