# Third-Party Attributions

This project uses third-party audio licensed under Creative Commons Attribution 4.0 International (CC BY 4.0) or Creative Commons Zero (CC0).

License CC BY 4.0: https://creativecommons.org/licenses/by/4.0/
License CC0: https://creativecommons.org/publicdomain/zero/1.0/

## Audio

### Music

- "Cloud Dancer" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer lobby (main game room / sign-in and game selection waiting area)
  - Changes: none

- "Farting Around" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer during `registry-19-shave-the-yak` active round gameplay
  - Changes: none

- "Zazie" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer during `registry-25-lowball-marketplace` active round gameplay
  - Changes: none

- "Pixelland" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer during `registry-04-escape-maze` active round gameplay
  - Changes: none

- "Vivacity" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer during `registry-17-fire-match-blow-shake` active round gameplay
  - Changes: none

- "Le Grand Chase" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer during `registry-14-dont-get-caught` active round gameplay
  - Changes: none

- "Thinking Music" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer during `registry-20-odd-one-out` and `registry-40-paint-match` active round gameplay
  - Changes: none

- "The Entertainer" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer throughout `registry-43-medical-story` once the game begins
  - Changes: none

- "Ouroboros" — Kevin MacLeod (incompetech.com)
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: TV viewer during `registry-10-grid-tap-colors` active round gameplay
  - Changes: none

### Sound Effects

- Cash Register — kiddpark
  - Source: https://freesound.org/s/201159/
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: staged asset (not yet wired to a game)
  - Changes: none

- Whip Crack 01.wav — CGEffex
  - Source: https://freesound.org/s/93100/
  - License: CC BY 4.0
  - License URL: https://creativecommons.org/licenses/by/4.0/
  - Used in: staged asset (not yet wired to a game)
  - Changes: none

- Men Shouting Hey — Jace + UncleSigmund
  - Sources: https://freesound.org/s/57204/ (Jace, CC0) and https://freesound.org/s/30989/ (UncleSigmund, CC BY 4.0)
  - License: CC0 (Jace portion) + CC BY 4.0 (UncleSigmund portion)
  - License URLs: https://creativecommons.org/publicdomain/zero/1.0/ and https://creativecommons.org/licenses/by/4.0/
  - Used in: staged asset (not yet wired to a game)
  - Changes: files combined into men_shouting_hey.wav

- Disco Clap — NenadSimic
  - Source: https://freesound.org/s/169085/
  - License: CC0
  - License URL: https://creativecommons.org/publicdomain/zero/1.0/
  - Used in: TV viewer during `registry-25-lowball-marketplace` Funny Messages sub-phase (plays on each player submission)
  - Changes: none

- Car Crash — squareal
  - Source: https://freesound.org/s/237375/
  - License: CC0
  - License URL: https://creativecommons.org/publicdomain/zero/1.0/
  - Used in: staged asset (not yet wired to a game)
  - Changes: none

- Whoosh — qubodup
  - Source: https://freesound.org/s/60013/
  - License: CC0
  - License URL: https://creativecommons.org/publicdomain/zero/1.0/
  - Used in: TV viewer during `registry-06-sound-replication` as the target reference sound players must replicate
  - Changes: none

## Models

- "Human Dude Guy" — hat_my_guy
  - Source: https://poly.pizza/m/83DpVkzvWk
  - License: Poly Pizza standard attribution request
  - Used in: `registry-43-medical-story` phone body-part selector and TV winner display
  - Changes: none

## Short-form in-app attribution

A shortened attribution line is displayed in the TV UI while each CC BY track is actively playing, with a link to the CC BY 4.0 license and this file for full details.
